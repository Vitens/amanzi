...
export const scenarioStore = (uid) => defineStore('scenario/'+uid, {
  state: () => ({
  ...
  actions: {
    ...
    findAllProductPaths() {
      ...
      const sourceNodes = this.models.filter(model => {
        const hasNoIncoming = !this.connections.some(conn => 
          conn.type === 'product' && conn.tgt === model.uid
        );
        const hasOutgoing = this.connections.some(conn =>
          conn.type === 'product' && conn.src === model.uid
        );
      ...
      const findPaths = (currentNode, currentPath, visited) => {
        ...
        const outgoing = this.connections.filter(conn => 
          conn.type === 'product' && conn.src === currentNode
        )
    ...
    copy(cut=false) {
      ...
      // if cut, push undo
      if(this.selectedBlocks.length > 0 && cut) { this.$pushUndo() }
      ...
      for(var conn of this.connections) {
        if(this.selectedBlocks.includes(conn.src) && this.selectedBlocks.includes(conn.tgt)) {
      ...
      if(cut) {
        this.models = this.models.filter(m => !this.selectedBlocks.includes(m.uid))
        this.connections = this.connections.filter(c => !this.selectedBlocks.includes(c.src) && !this.selectedBlocks.includes(c.tgt))
        this.selectedBlocks = []
      }
    },
    ...
    paste() {
      ...
      if(!this.copyOffset) { this.copyOffset = 20 }
    ...
    patchUndo(undo) {
      ...
      undo.models.forEach(m => {
        if(this.models.find(mdl => mdl.uid == m.uid)) {
          m.configuration = this.models.find(mdl => mdl.uid == m.uid).configuration
        }
      })
      // unselect all blocks
      this.$patch(undo)
    ...
    suggestName(name) {
      let names = []
      for(var m of this.models) { names.push(m.name)}
      if(!_.includes(names, name)) { return name }
    ...
    selectBlock(uid, append=false) {
      ...
      if(this.selectedBlocks.includes(uid)) { return }
      // if append is true, add to selection, otherwise replace selection
      if(!append) { this.deselectBlocks() }
        ...
        for(var anchor of modelspec.canvas.anchors) {
          ...
          for(var m of this.models) {
            ...
            for(var a of spec.canvas.anchors) {
              ...
              if(a.type != anchor.type || a.direction != target) { continue }
              // check if anchor is free
              if((a.direction == 'in' && this.connections.find(conn => conn.tgt == m.uid && conn.tgtAnchor == a.position)) ||  
                (a.direction == 'out' && this.connections.find(conn => conn.src == m.uid && conn.srcAnchor == a.position))) {
                continue
              }
          ...
          if(closestMatch) {
            ...
            let srcAnchor = anchor.direction == 'in' ? closestMatch.position : anchor.position
            let tgtAnchor = anchor.direction == 'in' ? anchor.position : closestMatch.position
    ...
    moveSelection(dx, dy) {
      ...
      for(var conn of this.connections) {
        if(this.selectedBlocks.includes(conn.src) && !this.selectedBlocks.includes(conn.tgt)) {
          conn.customPath = []
        } else if(!this.selectedBlocks.includes(conn.src) && this.selectedBlocks.includes(conn.tgt)) {
          conn.customPath = []
        }
      }
      ...
      for(var uid of this.selectedBlocks) {
        ...
        for(var conn of this.connections.filter(conn => conn.src == uid || conn.tgt == uid)) {
    ...
    deleteBlock(uid, deselect=true) {
      ...
      if(deselect) { this.$pushUndo() }

      // remove block from list of all models
      this.$patch((state) => {
        state.models = state.models.filter(block => block.uid != uid)
        state.connections = state.connections.filter(conn => (conn.src != uid && conn.tgt != uid))
      })
      if(deselect) { this.deselectBlocks() }
      this.unsolved = true
      this.unsaved = true
    },
    ...
    deselectConnections() {
      ...
      for(var c of this.connections) { c.selected = false }
    },
    ...
    deleteConnection() {
      ...
      if(this.connections.filter(conn => conn.selected).length == 0) { return }
    ...
    validate() {
      ...
      for(var m of this.models) {
        ...
        if(spec.source) { source = true }
        if(spec.sink) { sink = true }

      }
      
      if(!source && !sink) {
        return {valid: false, message: "ui.general.messages.missing_source_sink"}
      }
      if(!source) {
        return {valid: false, message: "ui.general.messages.missing_source"}
      }
      if(!sink) {
      ...
      for(var m of this.models) {
        ...
        for(var a of spec.canvas.anchors) {
          if(a.optional) { continue } // skip optional anchors
          var connections = []
          if(a.direction == 'in') {
            // check if all incoming connections are made
            connections = this.connections.filter(c => c.tgt == m.uid && c.tgtAnchor == a.position)
          } else {
            // check if all outgoing connections are made
            connections = this.connections.filter(c => c.src == m.uid && c.srcAnchor == a.position)
          }
          if(connections.length == 0) {
            return {valid: false, message: "ui.general.messages.missing_connections", data: {'name': m.name}}
          }
        }
      }
