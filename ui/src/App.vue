...
<div id="main">
  ...
  <el-dialog
    ...
    class="default"
  >
    <template v-if="designVisible" #header>
      <DesignHeader :title="dialogTitle"></DesignHeader>
    </template>
...
import ScenarioFeedback from './components/ScenarioFeedback.vue'
...
import { notifyMigrationIfNeeded } from './lib/migrationToast'

export default {
  name: 'App',
  components: {
    Canvas, Sidebar, Scenariobar, ScenarioFeedback, Design, Topbar, LoadingIndicator, Report, KeyFigures, ResultBar, Tutorial, About, DesignHeader,
  },
  data() { return {
    initialized: true,
    progress: {
  ...
  async mounted() {
    ...
    const onMigrationNotify = () => {
      notifyMigrationIfNeeded(this.$project, (key, params) => this.$t(key, params))
    }
    this.$bus.on('migration-notify', onMigrationNotify)
