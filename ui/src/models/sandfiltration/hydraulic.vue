<template>
    <svg>
        ...
        <path :d="sandLayer" fill="white" stroke="black" stroke-width="2"  />
    ...
    export default {
      name: 'hydraulic-sandfilter',    
      props: ['position', 'config', 'path' , 'dim'],
      mixins: [hydraulicLine],

      computed: {
      bottom() {
        return this.params.inlet_elevation-(this.params.fall_height_to_media + this.params.height_sand + this.params.filter_bottom + (this.params.dual_media ? this.params.height_anthracite : 0))
      },
      dimensions() {
        return {
      ...
      anchorpoints() {
        ...
        let outlet_y = this.y(this.params.inlet_elevation-this.params.fall_height_to_media - h_anthracite - this.params.height_sand - this.params.filter_bottom/2)
      ...
      sandLayer(){
        let sandHeight = this.params.height_sand*this.stepY;
        let ySand = this.y(this.params.inlet_elevation);
        if (this.params.dual_media){
          ySand = ySand + this.params.height_anthracite*this.stepY;
        }
        if (this.params.spray){
          ySand = ySand + this.params.fall_height_to_media*this.stepY;
          var sandLayer = this.Layer(this.x0+0.1*this.width, ySand, 0.8*this.width, sandHeight);
        }
        else if(this.params.pressurized){
          ySand = ySand + this.params.fall_height_to_media*this.stepY;
          var sandLayer = this.Layer(this.x0+0.1*this.width, ySand, 0.8*this.width, sandHeight);
          ...
          ySand = ySand + this.params.fall_height_to_media*this.stepY;
          var sandLayer = this.Layer(this.x0+0.2*this.width, ySand, 0.7*this.width, sandHeight);
        }
        return sandLayer;
      },
      filterdimensions(){
        let distance_to_top = 0.12 // m
        var filterheight =(this.params.height_sand+distance_to_top+this.params.filter_bottom+this.params.fall_height_to_media)*this.stepY;
